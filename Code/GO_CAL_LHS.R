...
ranges <- 
  list(lower = c(0.2,5,-100,0.1,0), 
       upper = c(0.6,50,0,0.8,3))
#------------------------------------------------------------------------------#
...
for (samp in 1:nsamps){
  ...
  for (i in 1:nt){
    ...
    if (yday(met$date[i])==243 & met$date[i]>spin_up_end){
      ...
      # If dA > glacier area, set area to 0, error metric to 
      # NA, and skip this time loop
      if (cmb_ann < 0 & sum(hyps[,2])<dA){
        print("[...Note: glacier disappeared!]")
        # Note: errors[samp] will remain NA (as init to this)
